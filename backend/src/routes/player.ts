...
router.post('/batch', enforceApiKeyAuth, enforceBatchRateLimit, async (req, res, next) => {
  ...
  try {
    ...
    // ⚡ Bolt: Use a pre-allocated array and a standard for loop instead of .map() inside Promise.all()
    // to eliminate O(N) intermediate array allocation and closure overhead natively generated by .map()
    const promises = new Array(uniqueUuids.length);
    for (let i = 0; i < uniqueUuids.length; i++) {
      const identifier = uniqueUuids[i];
      promises[i] = batchLimit(async () => {
        try {
          const startedAt = process.hrtime.bigint();
          const resolved = await resolvePlayer(identifier);
          const experience = extractBedwarsExperience(resolved.payload);
