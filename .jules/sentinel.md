...
**Prevention:** Use fast cryptographic operations for API key validation. We switched to **HMAC-SHA256** with a process-local random salt. This provides resistance against pre-computation attacks (should the config leak) while being orders of magnitude faster than PBKDF2, maintaining constant-time comparison to prevent timing attacks.
