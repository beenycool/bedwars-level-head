                ...
                ConfigProfiles.Preset.entries.forEach { preset ->
                    val line = ChatComponentText("${ChatColor.YELLOW}- ").appendSibling(
                        CommandUtils.createClickableCommand(
                            ...
                            hoverTextOverride = "${ChatColor.GREEN}Click to prepare to apply ${preset.name}"
                        )
...
private fun createShowSelfToggleComponent(showSelf: Boolean): IChatComponent {
    ...
    return ChatComponentText("$toggleColor$toggleText").apply {
