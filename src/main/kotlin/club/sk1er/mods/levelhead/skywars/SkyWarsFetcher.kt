    ...
    private suspend fun fetchFromProxy(identifier: String, lastFetchedAt: Long?, etag: String?): FetchResult {
        ...
        if (url == null) {
            Levelhead.logger.error(
                "Failed to build proxy SkyWars endpoint URL from base '{}'",
        ...
        return try {
            executeWithRetries(request, "proxy skywars").use { response ->
                ...
                val json = runCatching { Levelhead.jsonParser.parse(body).asJsonObject }.getOrElse {
                    Levelhead.logger.error("Failed to parse proxy SkyWars response", it)
            ...
            if (networkIssueWarned.compareAndSet(false, true)) {
                Levelhead.sendChat("${ChatColor.RED}SkyWars stats offline. ${ChatColor.YELLOW}Retrying in 60s.")
            }
            FetchResult.TemporaryError(ex.message)
        } catch (ex: Exception) {
            Levelhead.logger.error("Failed to fetch proxy SkyWars data", ex)
            FetchResult.TemporaryError(ex.message)
        }
    }
